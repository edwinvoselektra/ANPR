...
export async function openDigestStream(options: {
  ...
  username?: string; password?: string; signal: AbortSignal; timeoutMs: number; requireAuthentication?: boolean;
  ...
  if (response.statusCode !== 401) {
    ...
    if (options.requireAuthentication) { response.destroy(); throw new Error("AUTH_NOT_VERIFIED"); }
    return response;
  }
...
export function dahuaEventPath(channel: number) {
  if (!Number.isInteger(channel) || channel < 1 || channel > 64) throw new Error("INVALID_CHANNEL");
  return `/cgi-bin/snapManager.cgi?action=attachFileProc&channel=${channel}&heartbeat=5&Flags[0]=Event&Events=[TrafficJunction]`;
}
